...
@static if haskey(ENV,"JULIA_PETSC_LIBRARY")
  ...
  const libpetsc_provider = "JULIA_PETSC_LIBRARY"
  const libpetsc_path = ENV["JULIA_PETSC_LIBRARY"]
  const libpetsc_handle = Libdl.dlopen(libpetsc_path, flags)
  ...
  """
  using PETSc_jll
  const libpetsc_provider = "PETSc_jll"
  const libpetsc_path = PETSc_jll.libpetsc_path
  const libpetsc_handle = PETSc_jll.libpetsc_handle
