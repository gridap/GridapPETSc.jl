...
if isdir(PETSC_DIR)
    ...
    if !(PETSC_LIB === nothing) && isfile(PETSC_LIB)
        ...
        if petsc_real_datatype == PETSC_DOUBLE
            PETSC_SCALAR = PETSC_REAL = Float64
        elseif petsc_real_datatype == PETSC_FLOAT
            PETSC_SCALAR = PETSC_REAL = Float32
        else
...
open(deps_jl,"w") do f
  ...
  println(f, :(const PETSC_SCALAR      = $PETSC_SCALAR))
  println(f, :(const PETSC_REAL        = $PETSC_REAL))
  println(f, :(const PETSC_INT         = $PETSC_INT))
end
